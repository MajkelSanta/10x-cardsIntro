...
- `npx supabase start` / `npx supabase stop` — local Supabase stack (requires Docker; credentials printed on start)
- `npx wrangler deploy` — deploy to Cloudflare Workers (secrets via `wrangler secret put` or Cloudflare dashboard)

## 10xDevs Workflow Chain

Skills used in this project (run in order for new features):

```
/10x-init → /10x-shape → /10x-prd → /10x-tech-stack-selector → /10x-bootstrapper
→ /10x-agents-md → /10x-rule-review → /10x-lesson → /10x-infra-research → Plan Mode deploy
```

Context artifacts: `context/foundation/tech-stack.md`, `context/foundation/prd.md`, `context/foundation/infrastructure.md`, `context/deployment/deploy-plan.md`.

Skills must not write to `context/archive/` — archived changes are immutable.
